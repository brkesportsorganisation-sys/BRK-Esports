...
const SYSTEM_ROLES = [
  { role: 'USER', label: 'Player (User)', color: 'bg-slate-100 text-slate-700 border-slate-200' },
  { role: 'SUB_ADMIN', label: 'Sub Admin', color: 'bg-blue-100 text-blue-700 border-blue-200' },
  { role: 'MODERATOR', label: 'Moderator', color: 'bg-purple-100 text-purple-700 border-purple-200' },
  { role: 'VENDOR', label: 'Shop Vendor', color: 'bg-amber-100 text-amber-700 border-amber-200' },
  { role: 'ADMIN', label: 'Admin', color: 'bg-emerald-100 text-emerald-700 border-emerald-200' },
...
export default function AdminUsersPage() {
  ...
  const handleUpdateRole = async (userId: string, newRole: string, newInGameRole: string) => {
    setRoleSaving(true);
    try {
      ...
      if (res.ok) {
        alert(`User role updated to ${newInGameRole} / ${newRole} successfully in Database!`);
        setUsers((prev) =>
          prev.map((u) => (u.id === userId ? { ...u, role: newRole as any, inGameRole: newInGameRole } : u))
        );
        setInspectUser((prev) =>
          prev && prev.id === userId ? { ...prev, role: newRole as any, inGameRole: newInGameRole } : prev
        );
      ...
      alert('Error connecting to database to update role.');
    } finally {
      setRoleSaving(false);
    }
  };
            ...
            {inspectTab === 'ROLES' && (
              <div className="space-y-5 text-xs">
                ...
                <div className="p-5 rounded-2xl bg-slate-50 border border-slate-200 space-y-3">
                  <div className="flex items-center justify-between">
                    <div>
                      <h4 className="font-heading font-black text-sm text-slate-900">In-Game Esports Role</h4>
                      <p className="text-[11px] text-slate-500">Player's specialty shown in tournaments, squads, and profile badge.</p>
                    </div>
                    <span className="px-2.5 py-1 rounded-xl bg-orange-100 text-orange-800 font-bold text-xs">
                      Current: {ESPORTS_ROLES.find(r => r.role === (inspectUser.inGameRole || 'RUSHER'))?.label || inspectUser.inGameRole}
                    </span>
                  </div>

                  <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 pt-1">
                    {ESPORTS_ROLES.map((r) => {
                      const isSelected = (inspectUser.inGameRole || 'RUSHER') === r.role;
                      return (
                        <button
                          key={r.role}
                          onClick={() => handleUpdateRole(inspectUser.id, inspectUser.role || 'USER', r.role)}
                          disabled={roleSaving}
                ...
                <div className="p-5 rounded-2xl bg-slate-50 border border-slate-200 space-y-3">
                  <div className="flex items-center justify-between">
                    <div>
                      <h4 className="font-heading font-black text-sm text-slate-900">Platform System Role</h4>
                      <p className="text-[11px] text-slate-500">Determines dashboard access, administration permissions, and badge.</p>
                    </div>
                    <span className="px-2.5 py-1 rounded-xl bg-indigo-100 text-indigo-800 font-bold text-xs">
                      Current: {inspectUser.role || 'USER'}
                    </span>
                  </div>

                  <div className="grid grid-cols-2 sm:grid-cols-3 gap-2 pt-1">
                    {SYSTEM_ROLES.map((sr) => {
                      const isSelected = (inspectUser.role || 'USER') === sr.role;
                      return (
                        <button
                          key={sr.role}
                          onClick={() => handleUpdateRole(inspectUser.id, sr.role, inspectUser.inGameRole || 'RUSHER')}
                          disabled={roleSaving}
                          className={`p-3 rounded-xl border font-bold text-xs flex items-center justify-center gap-1.5 transition-all cursor-pointer ${
                            isSelected
            ...
            <div className="flex items-center justify-between border-b border-slate-100 pb-3">
              <div>
                <h3 className="font-heading font-black text-lg text-slate-900">
                  Update Role: {roleModal.userName}
                </h3>
                <p className="text-xs text-slate-500 font-mono">{roleModal.playerUniqueId}</p>
              </div>
              <button
            ...
            <div className="space-y-4 text-xs font-medium">
              ...
              <div>
                <label className="font-bold text-slate-800 block mb-1.5">
                  Select In-Game Role (Esports Badge):
                </label>
              ...
              <div>
                <label className="font-bold text-slate-800 block mb-1.5">
                  Select System Role (Platform Access):
                </label>
              ...
              <div className="p-3 rounded-xl bg-amber-50 border border-amber-200 text-[11px] text-amber-800 space-y-1">
                <span className="font-bold">⚡ Database Auto-Sync:</span>
                <p>This action updates the player's role directly in the PostgreSQL database.</p>
              </div>
              ...
              <div className="flex gap-2 pt-2">
                ...
                <button
                  type="button"
                  disabled={roleSaving}
                  onClick={() => handleUpdateRole(roleModal.userId, roleModal.systemRole, roleModal.inGameRole)}
                  className="flex-1 py-3 rounded-xl bg-gradient-to-r from-amber-500 to-orange-500 text-white font-bold flex items-center justify-center space-x-1 disabled:opacity-50 shadow-xs cursor-pointer hover:from-amber-600 hover:to-orange-600"
                >
                  {roleSaving ? <Loader2 className="w-4 h-4 animate-spin" /> : <span>SAVE ROLE TO DB</span>}
                </button>
              </div>
            </div>
          </div>
