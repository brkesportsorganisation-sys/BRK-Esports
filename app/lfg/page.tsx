            ...
            {filteredPosts.map((post) => (
              ...
              >
                <div>
                  ...
                  {/* Description Compact Clickable Preview */}
                  <button
                    type="button"
                    onClick={() => setSelectedPostDetails(post)}
                    className="w-full text-left mt-2 p-3 bg-[#F8FAFC] hover:bg-orange-50/60 rounded-2xl border border-slate-200/80 hover:border-brand-orange/40 transition-all group cursor-pointer"
                    title="Click to view full bio & requirements"
                  >
                    <p className="text-slate-600 text-xs leading-relaxed line-clamp-1 font-medium group-hover:text-slate-900">
                      &quot;{post.description}&quot;
                    </p>
                    <div className="flex items-center gap-1 text-[10px] text-brand-orange font-bold mt-1">
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/70 backdrop-blur-xs animate-in fade-in duration-150">
          <div className="bg-white rounded-3xl p-6 sm:p-8 max-w-lg w-full border border-slate-200 shadow-2xl space-y-5 animate-in zoom-in-95 duration-150 max-h-[90vh] overflow-y-auto">
            ...
            <div className="flex flex-wrap gap-2">
              <span className="text-xs font-bold px-3 py-1 rounded-xl bg-orange-50 text-brand-orange border border-orange-200">
                🎯 {selectedPostDetails.roleNeeded}
              </span>
              <span className="text-xs font-bold px-3 py-1 rounded-xl bg-slate-100 text-slate-700">
                🎮 {selectedPostDetails.gameMode.replace('_', ' ')}
              </span>
              {selectedPostDetails.winRate ? (
                <span className="text-xs font-bold px-3 py-1 rounded-xl bg-amber-50 text-amber-700 border border-amber-200 flex items-center gap-1">
                  <Award className="w-3.5 h-3.5" />
                  {selectedPostDetails.winRate}% Win Rate
                </span>
              ) : null}
              <span className={`px-3 py-1 rounded-xl text-xs font-bold uppercase ${
                selectedPostDetails.status === 'OPEN' ? 'bg-emerald-50 text-emerald-700 border border-emerald-200' : 'bg-slate-100 text-slate-600'
              }`}>
                ● {selectedPostDetails.status}
              </span>
            </div>

            {/* Full Requirements / Player Bio */}
            <div className="space-y-2">
              <label className="text-xs font-black font-heading text-slate-900 uppercase tracking-wider flex items-center gap-1.5">
                <FileText className="w-4 h-4 text-brand-orange" />
                <span>Full Bio &amp; Recruitment Requirements</span>
              </label>

              <div className="p-4 rounded-2xl bg-slate-50 border border-slate-200 text-slate-800 text-xs sm:text-sm leading-relaxed whitespace-pre-wrap font-medium">
            ...
            <div className="pt-2 flex items-center gap-3">
              <Link
                href={`/messages?sellerId=${selectedPostDetails.userId}`}
                className="flex-1 py-3 px-4 rounded-2xl bg-slate-900 hover:bg-slate-800 text-white font-bold text-xs sm:text-sm transition-all flex items-center justify-center space-x-2 shadow-sm"
              >
                <MessageSquare className="w-4 h-4 text-brand-orange" />
                <span>Chat / Message</span>
              </Link>

              {activeTab === 'SQUAD_LOOKING_FOR_PLAYER' && selectedPostDetails.status === 'OPEN' && (
                <button
                  type="button"
                  onClick={() => {
                  ...
                  className="flex-1 py-3 px-4 rounded-2xl bg-gradient-to-r from-brand-red to-brand-orange text-white font-heading font-black text-xs sm:text-sm shadow-sm hover:brightness-110 transition-all flex items-center justify-center space-x-1.5 cursor-pointer"
                >
