    ...
    return (
      <div className="min-h-screen bg-[#F8FAFC] text-slate-900 flex flex-col font-sans">
        ...
        <div className="flex-1 flex flex-col items-center justify-center p-6 text-center space-y-4">
          ...
          <h2 className="text-xl font-black text-slate-900">Squad Not Found</h2>
          <p className="text-xs text-slate-500">This squad may have been disbanded or removed.</p>
          <Link href="/teams" className="px-5 py-2.5 rounded-xl bg-gradient-to-r from-brand-red to-brand-orange text-white font-bold text-xs shadow-md">
            Back to Squads Hub
          </Link>
        </div>
        <Footer />
  ...
  return (
    <div className="min-h-screen bg-[#F8FAFC] text-slate-900 flex flex-col font-sans pb-20 lg:pb-12">
      ...
      <main className="flex-1 max-w-7xl w-full mx-auto px-4 sm:px-6 lg:px-8 py-6 sm:py-8 space-y-6 sm:space-y-8">
        ...
        {/* ── Squad Hero Profile Banner (Modern Light Canvas with Vibrant Header) ── */}
        <div className="relative rounded-3xl overflow-hidden bg-white border border-slate-200 shadow-sm text-slate-900">
          ...
          <div className="p-6 sm:p-8 bg-white relative z-10 space-y-6">
              ...
              <div className="flex flex-col sm:flex-row sm:items-end gap-5">
                ...
                <div className="space-y-1.5 pt-2 sm:pt-0">
                  <div className="flex items-center gap-2 flex-wrap">
                    <span className="px-2.5 py-0.5 rounded-lg bg-orange-100 border border-orange-200 text-orange-700 font-mono text-xs font-black">
                      [{squad.tag}]
                    </span>
                    <h1 className="text-2xl sm:text-3xl md:text-4xl font-black font-heading text-slate-900">
                      {squad.name}
                    </h1>
                  </div>
                  <p className="text-xs text-slate-600 max-w-xl leading-relaxed font-medium">
                    {squad.description || 'Official registered esports squad roster.'}
                  </p>
                  <div className="flex items-center gap-3 text-xs text-slate-500 font-mono pt-1">
                    <span>Leader: <strong className="text-slate-900">{squad.leaderName}</strong></span>
                    <span>•</span>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 pt-2">
              <div className="bg-slate-50 border border-slate-200 rounded-2xl p-3.5 text-center">
                <span className="text-[10px] text-slate-500 font-bold uppercase tracking-wider">Active Roster</span>
                <div className="text-xl font-black text-slate-900 font-mono mt-0.5">{activeMembers.length} / 6</div>
              </div>

              <div className="bg-slate-50 border border-slate-200 rounded-2xl p-3.5 text-center">
                <span className="text-[10px] text-slate-500 font-bold uppercase tracking-wider">Matches Played</span>
                <div className="text-xl font-black text-orange-600 font-mono mt-0.5">{squad.matchesPlayed}</div>
              </div>

              <div className="bg-slate-50 border border-slate-200 rounded-2xl p-3.5 text-center">
                <span className="text-[10px] text-slate-500 font-bold uppercase tracking-wider">Tournament Wins</span>
                <div className="text-xl font-black text-emerald-600 font-mono mt-0.5">{squad.matchesWon}</div>
              </div>

              <div className="bg-slate-50 border border-slate-200 rounded-2xl p-3.5 text-center">
                <span className="text-[10px] text-slate-500 font-bold uppercase tracking-wider">Total Kills</span>
                <div className="text-xl font-black text-cyan-600 font-mono mt-0.5">{squad.totalKills}</div>
              </div>
            </div>
        ...
        {activeTab === 'ROSTER' && (
          <div className="space-y-6">
            ...
            {/* Minimum 4 Players Notification Banner for Tournament Registration */}
            {activeMembers.length < 4 && (
              <div className="p-4 rounded-2xl bg-gradient-to-r from-amber-500/10 via-orange-500/10 to-amber-500/10 border border-amber-300 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                <div className="flex items-start gap-3">
                  ...
                  <div className="space-y-0.5">
                    <h4 className="text-xs sm:text-sm font-bold text-amber-950">
                      Roster Requirement: {activeMembers.length} / 4+ Players
                    </h4>
                    <p className="text-[11px] sm:text-xs text-amber-800">
                      You need at least <strong>4 active players</strong> in your squad to register for Giveaway and official Battle Royale tournaments.
                    </p>
                  </div>
                </div>
                ...
                <div className="flex items-center gap-2 shrink-0">
                  <button
                    onClick={() => setLinkModalOpen(true)}
                    className="px-3 py-1.5 rounded-xl bg-white hover:bg-amber-50 text-amber-950 text-xs font-bold border border-amber-300 shadow-2xs cursor-pointer flex items-center gap-1.5"
                  >
                    <Share2 className="w-3.5 h-3.5 text-brand-orange" />
                    <span>Copy Link</span>
                  </button>
                  {canManage && (
                    <button
                      onClick={() => setInviteModalOpen(true)}
                      className="px-3.5 py-1.5 rounded-xl bg-gradient-to-r from-brand-red to-brand-orange text-white text-xs font-black shadow-xs cursor-pointer flex items-center gap-1.5"
                    >
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
              {activeMembers.map((member) => {
                ...
                const isSelf = member.userId === currentUser?.id;
                const appId = member.accountNumber || `EZBD-${(member.userId || '').substring(0, 6).toUpperCase()}`;
                ...
                return (
                  ...
                  >
                    <div className="space-y-3">
                      {/* Player Profile Top Row */}
                      <div className="flex items-start justify-between gap-3">
                        <div className="flex items-center gap-3">
                          <img
                            src={member.userAvatar || `https://api.dicebear.com/7.x/bottts/svg?seed=${member.userName}`}
                            alt={member.userName}
                            className="w-12 h-12 rounded-2xl object-cover border-2 border-slate-200 bg-slate-50"
                          />
                          <div>
                            <div className="flex items-center gap-1.5">
                              <h4 className="font-black text-slate-900 text-sm">{member.userName}</h4>
                              {isMemberLeader && <span title="Squad Leader">👑</span>}
                            </div>
                            <div className="text-[11px] text-slate-500 font-mono mt-0.5">
                              App ID: <strong className="text-slate-800">{appId}</strong>
                            </div>
                          </div>
                        </div>
                        ...
                        <span className={`px-2.5 py-1 rounded-full text-[10px] font-black uppercase border ${
                          ...
                          {isMemberLeader ? 'LEADER' : member.memberType}
                        </span>
                      </div>

                      {/* In-Game Role, App ID & Game UID Details */}
                      <div className="bg-slate-50 p-3.5 rounded-2xl border border-slate-200 space-y-2">
                        
                        {/* App ID with 1-Click Copy */}
                        <div className="flex items-center justify-between text-xs">
                          ...
                          <div className="flex items-center gap-1.5 font-mono">
                            <strong className="text-slate-900 font-bold">{appId}</strong>
                            <button
                              type="button"
                              onClick={() => handleCopyText(appId, `app_${member.id}`)}
                              ...
                              title="Copy App ID"
                            >
                              {copiedKey === `app_${member.id}` ? <Check className="w-3 h-3 text-emerald-600" /> : <Copy className="w-3 h-3" />}
                            </button>
                          </div>
                        </div>
                      ...
                      <button
                        ...
                        className="w-full py-2 px-3 rounded-xl bg-slate-100 hover:bg-slate-200 text-slate-800 text-xs font-heading font-black flex items-center justify-center gap-1.5 border border-slate-200 transition-all cursor-pointer shadow-2xs active:scale-98"
                      >
                    ...
                    <div className="pt-2 flex items-center justify-end gap-2 border-t border-slate-100">
                      {canManage && !isMemberLeader && (
                        <button
                          ...
                          className="px-3 py-1.5 bg-slate-100 hover:bg-slate-200 text-slate-700 text-xs font-bold rounded-xl flex items-center gap-1 border border-slate-200 cursor-pointer transition-colors"
                        >
                      ...
                      {isLeader && !isMemberLeader && (
                        <button
                          ...
                          className="px-3 py-1.5 bg-orange-50 hover:bg-orange-100 text-orange-700 text-xs font-bold rounded-xl border border-orange-200 cursor-pointer transition-colors"
                      ...
                      {(canManage || isSelf) && (
                        <button
                          ...
                          className="px-3 py-1.5 bg-red-50 hover:bg-red-100 text-red-600 text-xs font-bold rounded-xl border border-red-200 cursor-pointer transition-colors"
                        >
              ...
              {/* Open / Empty Squad Slots Placeholder Cards */}
              {Array.from({ length: Math.max(0, 6 - activeMembers.length) }).map((_, idx) => {
                const slotNum = activeMembers.length + idx + 1;
                return (
                  <div
                    key={`empty_slot_${slotNum}`}
                    className="bg-slate-50/70 border-2 border-dashed border-slate-200 rounded-3xl p-5 flex flex-col justify-between items-center text-center space-y-4 hover:border-brand-orange/40 transition-colors"
                  >
                    <div className="space-y-2 py-4">
                      ...
                      <div>
                        <h4 className="font-bold text-slate-700 text-sm">Squad Slot #{slotNum}</h4>
                        <p className="text-[11px] text-slate-400">Empty Player Slot</p>
                      </div>
                    </div>
                    ...
                    {canManage ? (
                      <button
                        type="button"
                        onClick={() => setInviteModalOpen(true)}
                        className="w-full py-2 px-3 rounded-xl bg-white hover:bg-orange-50 text-brand-orange hover:text-orange-700 text-xs font-bold border border-orange-200 transition-colors cursor-pointer shadow-2xs flex items-center justify-center gap-1.5"
                      >
                      ...
                      <div className="text-[10px] text-slate-400 font-medium py-1">
                        Waiting for leader to add player
                      </div>
                    )}
                  </div>
                );
              ...
              {pendingRequests.length === 0 ? (
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  {pendingRequests.map((req) => (
                    <div key={req.id} className="bg-slate-50 border border-slate-200 rounded-2xl p-4 flex items-center justify-between gap-4">
                      <div className="flex items-center gap-3">
                        <img src={req.userAvatar} alt={req.userName} className="w-10 h-10 rounded-xl object-cover bg-white border border-slate-200" />
                        <div>
                          <h4 className="font-bold text-slate-900 text-sm">{req.userName}</h4>
                          <span className="text-[11px] text-slate-500 font-mono">Account: {req.accountNumber}</span>
                          <div className="text-[10px] text-orange-600 font-bold mt-0.5">Role: {req.inGameRole}</div>
                        </div>
                      </div>
                      ...
                      <div className="flex items-center gap-2 shrink-0">
                        ...
                        <button
                          ...
                          className="px-3 py-1.5 bg-slate-100 hover:bg-red-50 text-slate-600 hover:text-red-600 rounded-xl text-xs font-bold border border-slate-200 cursor-pointer"
                        >
                          Reject
                        </button>
                      </div>
              ...
              {invitedMembers.length === 0 ? (
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  {invitedMembers.map((inv) => (
                    <div key={inv.id} className="bg-slate-50 border border-slate-200 rounded-2xl p-4 flex items-center justify-between gap-4">
                      <div className="flex items-center gap-3">
                        <img src={inv.userAvatar} alt={inv.userName} className="w-10 h-10 rounded-xl object-cover bg-white border border-slate-200" />
                        <div>
                          <h4 className="font-bold text-slate-900 text-sm">{inv.userName}</h4>
                          <span className="text-[11px] text-slate-500 font-mono">Invited as {inv.inGameRole}</span>
                        </div>
                      </div>
        ...
        {activeTab === 'SETTINGS' && canManage && (
          <div className="space-y-6 max-w-3xl">
            <form onSubmit={handleSaveSettings} className="bg-white border border-slate-200 rounded-3xl p-6 sm:p-8 space-y-5 text-xs font-medium shadow-sm">
              <div className="border-b border-slate-100 pb-3">
                <h3 className="text-lg font-black font-heading text-slate-900">Edit Squad Profile & Branding</h3>
                <p className="text-xs text-slate-500">Update squad name, tag code, banner image, and recruitment settings.</p>
              </div>
              ...
              <div className="pt-2">
                <label className="flex items-center gap-2.5 cursor-pointer">
                  <input
                    ...
                    className="text-brand-orange focus:ring-orange-400 rounded cursor-pointer"
                  />
              ...
              <div className="pt-3 border-t border-slate-100 flex items-center justify-end">
                <button
                  ...
                  className="px-6 py-2.5 bg-gradient-to-r from-brand-red to-brand-orange hover:brightness-110 text-white font-heading font-black text-xs uppercase rounded-xl shadow-md cursor-pointer disabled:opacity-50 flex items-center gap-2"
                >
            ...
            {isLeader && (
              <div className="bg-red-50 border border-red-200 rounded-3xl p-6 space-y-3">
                <div className="flex items-center gap-2 text-red-600 font-black font-heading text-sm uppercase">
          ...
          <div className="bg-white border border-slate-200 rounded-3xl p-6 sm:p-8 max-w-lg w-full space-y-5 shadow-2xl text-slate-900 max-h-[90vh] overflow-y-auto custom-scrollbar">
            ...
            <div className="flex items-center justify-between border-b border-slate-100 pb-3">
              <div className="flex items-center gap-2.5">
                ...
                <h3 className="font-heading font-black text-lg text-slate-900">Invite Player to Squad</h3>
              </div>
              <button
            ...
            <form onSubmit={handleSendInvite} className="space-y-4 text-xs font-medium">
              ...
              <div className="space-y-2">
                ...
                <div className="relative">
                  ...
                  <input
                    ...
                    className="w-full pl-10 pr-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-xs text-slate-900 placeholder:text-slate-400 focus:outline-none focus:border-brand-orange"
                  />
                  {isSearching && (
                ...
                {searchResults.length > 0 && !selectedPlayer && (
                  <div className="bg-slate-50 border border-slate-200 rounded-2xl p-2 space-y-1 max-h-44 overflow-y-auto">
                    {searchResults.map((p) => (
                      <button
                        key={p.id}
                        type="button"
                      ...
                      >
                        <div className="flex items-center gap-2.5">
                          <img src={p.avatar} alt={p.name} className="w-8 h-8 rounded-lg object-cover bg-white border border-slate-200" />
                          <div>
                            <div className="font-bold text-slate-900 text-xs">{p.name}</div>
                            <div className="text-[10px] text-slate-500 font-mono">Account: {p.accountNumber}</div>
                          </div>
                        </div>
                        <span className="px-2 py-1 rounded-md bg-orange-100 text-orange-700 text-[10px] font-bold">Select</span>
                      </button>
                    ))}
                  </div>
                )}
                ...
                {selectedPlayer && (
                  <div className="bg-orange-50 border border-orange-200 rounded-2xl p-3 flex items-center justify-between gap-3">
                    <div className="flex items-center gap-2.5">
                      <img src={selectedPlayer.avatar} alt={selectedPlayer.name} className="w-10 h-10 rounded-xl object-cover bg-white border border-orange-200" />
                      <div>
                        <div className="font-bold text-slate-900 text-xs">{selectedPlayer.name}</div>
                        <div className="text-[10px] text-slate-500 font-mono">Account: {selectedPlayer.accountNumber}</div>
                        {selectedPlayer.freeFireUid && (
                          <div className="text-[10px] text-emerald-600 font-mono">UID: {selectedPlayer.freeFireUid}</div>
                        )}
                      </div>
                    </div>
              ...
              <div className="pt-2 flex items-center justify-end gap-2 border-t border-slate-100">
                ...
                <button
                  ...
                  className="px-6 py-2.5 bg-gradient-to-r from-brand-red to-brand-orange hover:brightness-110 text-white font-heading font-black text-xs uppercase rounded-xl shadow-md cursor-pointer disabled:opacity-50 flex items-center gap-2 transition-all"
                >
      ...
      {linkModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/60 backdrop-blur-sm animate-fadeIn">
          <div className="bg-white border border-slate-200 rounded-3xl p-6 sm:p-8 max-w-md w-full space-y-5 shadow-2xl text-slate-900">
            ...
            <div className="flex items-center justify-between border-b border-slate-100 pb-3">
              <div className="flex items-center gap-2.5">
                ...
                <h3 className="font-heading font-black text-lg text-slate-900">Shareable Squad Invite Link</h3>
              </div>
              <button
            ...
            <div className="bg-slate-50 border border-slate-200 rounded-2xl p-3 flex items-center justify-between gap-2">
              <input
                ...
                className="bg-transparent text-xs font-mono text-slate-900 font-bold w-full focus:outline-none"
              />
              <button
                onClick={() => {
                ...
                className="px-3 py-1.5 bg-gradient-to-r from-brand-red to-brand-orange text-white rounded-xl text-xs font-black shrink-0 flex items-center gap-1 shadow-sm cursor-pointer"
              >
            ...
            <div className="space-y-2">
              ...
              {canManage && (
                <button
                  ...
                  className="w-full py-2.5 rounded-xl bg-slate-100 hover:bg-slate-200 text-slate-700 text-xs font-bold flex items-center justify-center gap-1.5 border border-slate-200 cursor-pointer transition-colors"
                >
          ...
          <div className="bg-white border border-slate-200 rounded-3xl p-6 sm:p-8 max-w-md w-full space-y-5 shadow-2xl text-slate-900">
            ...
            <div className="flex items-center justify-between border-b border-slate-100 pb-3">
              <div className="flex items-center gap-2.5">
                ...
                <h3 className="font-heading font-black text-lg text-slate-900">Change Role for {selectedMember.userName}</h3>
              </div>
              <button
            ...
            <form onSubmit={handleUpdateMemberRole} className="space-y-4 text-xs font-medium">
              <div className="space-y-1">
                <label className="text-slate-700 font-bold uppercase block text-[11px]">Designation Type *</label>
                <select
                  value={editType}
              ...
              <div className="space-y-1">
                <label className="text-slate-700 font-bold uppercase block text-[11px]">In-Game Role *</label>
                <select
                  value={editRole}
      ...
      {inspectingPlayerId && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/60 backdrop-blur-md animate-fadeIn overflow-y-auto">
          <div className="bg-white border border-slate-200 rounded-[2.5rem] p-6 sm:p-8 max-w-2xl w-full space-y-6 shadow-2xl text-slate-900 my-8 max-h-[90vh] overflow-y-auto">
            ...
            <div className="flex items-center justify-between border-b border-slate-100 pb-4">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h3 className="font-heading font-black text-lg sm:text-xl text-slate-900">Player Esports Profile</h3>
                  <p className="text-xs text-slate-500 font-medium">Career stats, kills, wins & tournament position records</p>
                </div>
              </div>
              <button
                onClick={() => {
                ...
                className="w-8 h-8 rounded-full bg-slate-100 hover:bg-slate-200 flex items-center justify-center text-slate-500 font-bold cursor-pointer transition-colors"
              >
                ✕
              </button>
            </div>
            ...
            {isLoadingPlayerStats ? (
              <div className="py-20 text-center space-y-3">
                ...
                <p className="text-xs text-slate-500 font-bold">Loading player career & tournament records...</p>
              </div>
            ) : playerStatsData?.player ? (
              <div className="space-y-6">
                
                {/* Player Identity Card */}
                <div className="bg-slate-50 border border-slate-200 rounded-3xl p-5 flex flex-col sm:flex-row items-center sm:items-start justify-between gap-4">
                  <div className="flex flex-col sm:flex-row items-center gap-4 text-center sm:text-left">
                    <img
                      src={playerStatsData.player.avatar}
                      alt={playerStatsData.player.name}
                      className="w-16 h-16 rounded-2xl object-cover border-2 border-orange-400 shadow-sm bg-white shrink-0"
                    />
                    <div className="space-y-1">
                      <div className="flex items-center justify-center sm:justify-start gap-2">
                        ...
                        <span className="px-2 py-0.5 rounded-md bg-orange-100 border border-orange-200 text-orange-700 text-[10px] font-black uppercase">
                          {playerStatsData.player.role}
                        </span>
                      </div>
                      <div className="text-xs text-slate-500 flex flex-wrap items-center justify-center sm:justify-start gap-3 font-medium">
                        <span>Account: <strong className="text-slate-800 font-mono">{playerStatsData.player.accountNumber}</strong></span>
                        <span>FF UID: <strong className="text-emerald-600 font-mono">{playerStatsData.player.freeFireUid}</strong></span>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="text-center sm:text-right shrink-0">
                    ...
                    <div className="text-xs font-black text-orange-600 font-heading">ACTIVE ROSTER MEMBER</div>
                  </div>
                </div>
                ...
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                  <div className="bg-slate-50 p-3.5 rounded-2xl border border-slate-200 text-center space-y-0.5">
                    ...
                    <div className="text-lg font-black text-slate-900 font-heading">{playerStatsData.player.matchesPlayed}</div>
                  </div>
                  <div className="bg-slate-50 p-3.5 rounded-2xl border border-slate-200 text-center space-y-0.5">
                    <div className="text-[10px] font-bold text-slate-500 uppercase">Total Kills</div>
                    <div className="text-lg font-black text-brand-red font-heading">{playerStatsData.player.totalKills}</div>
                  </div>
                  <div className="bg-slate-50 p-3.5 rounded-2xl border border-slate-200 text-center space-y-0.5">
                    <div className="text-[10px] font-bold text-slate-500 uppercase">Booyah Wins</div>
                    <div className="text-lg font-black text-orange-600 font-heading">{playerStatsData.player.totalWins}</div>
                  </div>
                  <div className="bg-slate-50 p-3.5 rounded-2xl border border-slate-200 text-center space-y-0.5">
                    <div className="text-[10px] font-bold text-slate-500 uppercase">Earnings</div>
                    <div className="text-lg font-black text-emerald-600 font-heading">৳{playerStatsData.player.earnings}</div>
                  </div>
                </div>
                ...
                <div className="space-y-3">
                  <div className="flex items-center justify-between border-b border-slate-100 pb-2">
                    <h4 className="text-xs sm:text-sm font-heading font-black text-slate-900 flex items-center gap-2">
                  ...
                  {playerStatsData.matchHistory && playerStatsData.matchHistory.length > 0 ? (
                    <div className="space-y-2.5 max-h-60 overflow-y-auto pr-1">
                      {playerStatsData.matchHistory.map((m: any) => (
                        <div
                          key={m.id}
                          className="p-3.5 rounded-2xl bg-slate-50 border border-slate-200 hover:border-slate-300 transition-all flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3"
                        >
                          <div className="space-y-1 min-w-0">
                            <div className="flex items-center gap-2">
                              <span className="px-2 py-0.5 rounded-md bg-orange-100 text-orange-700 font-black text-[10px] uppercase font-mono">
                                {m.gameMode}
                              </span>
                              <h5 className="font-bold text-xs text-slate-900 truncate">{m.tournamentTitle}</h5>
                            </div>
                            <div className="text-[11px] text-slate-500 flex items-center gap-3 font-medium">
                              <span>Map: {m.map}</span>
                              <span>Prize Pool: <strong className="text-emerald-600">৳{m.prizePool}</strong></span>
                            </div>
                          </div>
