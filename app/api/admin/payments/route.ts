...
export async function PATCH(request: NextRequest) {
  ...
  try {
    ...
    const wasAutoCredited = payment.notes?.includes('[Auto-Credited') || 
      (depositAmount <= 100 && !payment.notes?.includes('NOT Auto-Credited') && !payment.notes?.includes('Manual Approval'));

    if (action === 'APPROVE') {
      // 1. If NOT auto-credited (e.g. > ৳100), credit the player's wallet now
      if (!wasAutoCredited) {
